      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="local@dev.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="local@dev.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
